...
function createThreeCanvas() {
    ...
    CANVAS_WIDTH = canvas.scrollWidth;
    CANVAS_HEIGHT = canvas.scrollHeight;
    ...
    // Create a camera
    camera = new THREE.PerspectiveCamera(30, CANVAS_ASPECT, 0.1, 1000);
    ...
    cube = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.6, 0.3), material);
    cube.position.x = 0.5;
    cube.position.y = 0.5;
    cube.position.z = 0.5;
    scene.add(cube);
...
function onWindowResize() {
    var layer1 = document.getElementById('canvas');
...
function doTest() {
    ...
    var noise = function () {
        return random(-noiseLevel, +noiseLevel);
    };
    gx += noise();
    gy += noise();
    gz += noise();
