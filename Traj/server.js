...
handleMsgFromClients : function( server, address, args, socket) {
	if(args.type === 'osc') {
        ...
        if( split[1] === 'clients') { // message is addressed to another client
        	...
        	if(server.outputType === 'OSC') {
        		if(args.address=== "/savenimtraj")
        		{
        			savetraj(args.parameters);
        		}else{
 ...
 var __admin = {
	...
	if( global.interface.livecodeServer === null) {
		...
		if( options.type === 'WebSocket' ) {
			...
			srv = global.interface.makeServer({
				webSocketPort:  8081,
				oscInputPort: parameters[1] || 10000,
...
function savetraj(args){
  ...
  var content = "";
  //preamble
  content += "$___save_then_load_value____system_dummy_variable___ := NIM { \n";

  //content loop
  //format is dt TAB[x,y,z], \n

  var prevt = 0;
  var dt = 0;
  for (var ii = 2; ii< args.length; ii+=4){
  	dt = args[ii+3] - prevt;
  	content += dt + " TAB[" + args[ii] + ", " + args[ii+1] + ", " + args[ii+2] + "]"; 
  	if(ii === args.length - 4){
  		content+= "\n";
  	}else{
  		content+= ", \n";
  ...
  fs.writeFile(filename, content, function(err) {
  	...
  	console.log("The file saved --> " + filename);
  }); 
