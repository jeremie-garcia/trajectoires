...
//sending
- (void)sendMessageNoDouble:(CDVInvokedUrlCommand*)command
{
    CDVPluginResult* pluginResult = nil;
    ...
    if(osc==nil){
        ...
        message.address = [command.arguments objectAtIndex:2];
        for(int i=3; i<[command.arguments count]; i++){
            if ( [[command.arguments objectAtIndex:i] isKindOfClass:[NSDecimalNumber class]] )
            {   
                NSNumber num = [command.arguments objectAtIndex:i]
