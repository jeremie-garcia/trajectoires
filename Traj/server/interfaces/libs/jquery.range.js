...
(function($, window, document, undefined) {
	...
	jRange.prototype = {
		...
		onDrag: function(pointer, e) {
			...
			if (evt.constructor.name == "TouchEvent" || evt.constructor.toString() == "[object TouchEventConstructor]") {
				for (var k=0; k<evt.touches.length;k++) {
					//console.log(e.originalEvent.touches[k].identifier,e.originalEvent,e )
					if (evt.touches[k].identifier == this.timeSliderTouchId) {
						if (evt.touches ) {//&& evt.touches.length) {
							e = evt.touches[k];
						} else if (evt.touches ) { //&& evt.changedTouches.length) {
							e = evt.touches[k];
						}

						if (this.options.isVertical) {
							var position = e.clientY - this.domNode.offset().top;
						} else {
		...
		onDragEnd: function(e) {
			if (e.originalEvent.constructor.name == "TouchEvent") {
				for (var k=0; k<e.originalEvent.changedTouches.length;k++) {
					if (e.originalEvent.changedTouches[k].identifier == this.timeSliderTouchId) {
		...
		showPointerValue: function(pointer, position, animate) {
			...
			if ($.isFunction(this.options.format)) {
				...
				if (this.options.isVertical) {
					text = this.options.format.replace('%s', parseFloat(value).toFixed(2));
				} else {
